...
{
  EV_INFO << "veins radio driver handlemessage" << endl;
    if (msg == mChannelLoadReport) {
      ...
      EV_INFO << "encapsulating..." << endl;
        handleUpperMessage(msg); //上位層からのメッセージをencapsulationして下の層へ送る
    } else if (msg->getArrivalGate() == mLowerLayerIn) {
      EV_INFO << "decapsulating..." << endl;
        handleLowerMessage(msg); //下位層からのメッセージをdecapsulationして上の層へ送る
    } else {
